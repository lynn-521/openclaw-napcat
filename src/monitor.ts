...
export type NapCatMonitorOptions = {
  ...
  /** NapCat WebSocket server URL for forward connection. */
  wsUrl: string;
...
/**
 * Connect to NapCat's WebSocket server (forward WS mode).
 * OpenClaw initiates the connection to the NapCat OneBot 11 WS server.
 */
export async function monitorNapCatProvider(options: NapCatMonitorOptions): Promise<void> {
  const { account, config, runtime, abortSignal, wsUrl, statusSink } = options;
  const core = getNapCatRuntime();

  let reconnectDelay = 1000;
  const MAX_RECONNECT_DELAY = 30000;

  runtime.log?.(`[${account.accountId}] NapCat connecting to forward WS at ${wsUrl}`);
  ...
  const connect = (): void => {
    ...
    ws.on("open", () => {
      runtime.log?.(`[${account.accountId}] NapCat WS connected to ${wsUrl}`);
      reconnectDelay = 1000; // reset backoff on successful connection
      // Register this WS as the global send target (WS-first for outbound)
      setNapCatWs(ws, account.accessToken);
    });

    ws.on("message", (raw: Buffer) => {
      // First, check if this is an outbound API response (echo-matched).
      // If so, processWsApiResponse resolves/rejects the pending call and returns non-null.
      const apiResp = processWsApiResponse(raw);
      if (apiResp !== null) {
        // It was an API response — no further event processing needed.
        return;
      }
    ...
    ws.on("close", () => {
      runtime.log?.(`[${account.accountId}] NapCat WS disconnected, reconnecting in ${reconnectDelay}ms`);
      activeWs = null;
      clearNapCatWs();
      if (!aborted) {
  ...
  return new Promise<void>((resolve) => {
    const onAbort = () => {
      runtime.log?.(`[${account.accountId}] NapCat stopping WS client`);
      aborted = true;
      if (activeWs) {
        activeWs.close();
        activeWs = null;
...
async function processMessage(
      ...
      await issuePairingChallenge({
        ...
        sendPairingReply: async (replyText) => {
          try {
            ...
            if (isNapCatWsConnected()) {
              try {
                await wsSendPrivateMsg(numericId, [textSegment(replyText)]);
                statusSink?.({ lastOutboundAt: Date.now() });
                return;
              } catch (wsErr) {
                runtime.log?.(`[${account.accountId}] WS pairing reply failed, falling back to HTTP: ${String(wsErr)}`);
              }
            }
            // HTTP fallback
            await sendPrivateMsg(account.httpApi, numericId, [textSegment(replyText)], account.accessToken);
            statusSink?.({ lastOutboundAt: Date.now() });
          } catch (err) {
  ...
  // Rate limiting check — reject excessive requests before AI processing
  if (account.config.rateLimit?.enabled !== false) {
    const limiter = new RateLimiter(account.config);
    const numericSenderId = Number(senderId);
    const numericGroupId = isGroup ? Number(event.group_id) : undefined;
    const limitError = limiter.check(numericSenderId, numericGroupId);
    if (limitError) {
      runtime.log?.(`[${account.accountId}] Rate limited sender=${senderId}: ${limitError}`);
      // Send a friendly rejection back to user
      try {
        if (isNapCatWsConnected()) {
          try {
            if (isGroup) {
              await wsSendGroupMsg(Number(chatId), [textSegment(limitError)]);
            } else {
              await wsSendPrivateMsg(numericSenderId, [textSegment(limitError)]);
            }
            statusSink?.({ lastOutboundAt: Date.now() });
            return;
          } catch (wsErr) {
            runtime.log?.(`[${account.accountId}] WS rate limit reply failed, falling back to HTTP: ${String(wsErr)}`);
          }
        }
        // HTTP fallback
        if (isGroup) {
          await sendGroupMsg(account.httpApi, Number(chatId), [textSegment(limitError)], account.accessToken);
        } else {
          await sendPrivateMsg(account.httpApi, numericSenderId, [textSegment(limitError)], account.accessToken);
        }
        statusSink?.({ lastOutboundAt: Date.now() });
      } catch {
  ...
  // Set per-message sender context for admin guard access in tool execute wrappers
  const numericSenderId = Number(senderId);
  const numericGroupId = isGroup ? Number(event.group_id) : undefined;
  setCurrentSenderContext(numericSenderId, numericGroupId);
...
async function deliverNapCatReply(params: {
  ...
  // Helper: send a message via WS (preferred) or HTTP fallback.
  const send = async (segments: OneBotSegment[]) => {
    const numericId = Number(chatId);
    if (isNapCatWsConnected()) {
      try {
        if (isGroup) {
          await wsSendGroupMsg(numericId, segments);
        } else {
          await wsSendPrivateMsg(numericId, segments);
        }
        statusSink?.({ lastOutboundAt: Date.now() });
        return;
      } catch (err) {
        // WS send failed — log and fall through to HTTP
        runtime.log?.(`[${account.accountId}] WS send failed, falling back to HTTP: ${String(err)}`);
      }
    }
    // HTTP fallback
    if (isGroup) {
      await sendGroupMsg(account.httpApi, numericId, segments, account.accessToken);
    } else {
      await sendPrivateMsg(account.httpApi, numericId, segments, account.accessToken);
    }
    statusSink?.({ lastOutboundAt: Date.now() });
  };
  ...
  for (const url of fileUrls) {
    ...
    try {
      if (isNapCatWsConnected()) {
        try {
          if (isGroup) {
            await wsUploadGroupFile(numericId, url, name);
          } else {
            await wsUploadPrivateFile(numericId, url, name);
          }
          statusSink?.({ lastOutboundAt: Date.now() });
          continue;
        } catch (err) {
          runtime.log?.(`[${account.accountId}] WS file upload failed, falling back to HTTP: ${String(err)}`);
        }
      }
      // HTTP fallback
      if (isGroup) {
        await uploadGroupFile(account.httpApi, numericId, url, name, account.accessToken);
      } else {
        await uploadPrivateFile(account.httpApi, numericId, url, name, account.accessToken);
      }
      statusSink?.({ lastOutboundAt: Date.now() });
    } catch (err) {
