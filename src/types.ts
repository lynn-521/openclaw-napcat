...
export type NapCatAccountConfig = {
  ...
  /** NapCat OneBot 11 WebSocket server URL for forward connection (e.g. ws://127.0.0.1:3002). */
  wsUrl?: string;
  /** Rate limit configuration. */
  rateLimit?: {
    /** Enable rate limiting. */
    enabled?: boolean;
    /** Max messages per minute per user+group. */
    maxPerMinute?: number;
    /** Max messages per hour per user+group. */
    maxPerHour?: number;
    /** Max messages per day per user+group. */
    maxPerDay?: number;
  };
